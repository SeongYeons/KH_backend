/*
  <ALTER>
    오라클 데이터베이스 객체(테이블, 인덱스, 뷰 등)를 수정하는 구문이다.
*/
-- 실습에 사용할 테이블 생성
...
/*
      2) 컬럼 수정
        ...
        기본값 변경
          ALTER TABLE 테이블명 MODIFY 컬럼명 DEFALUT 변경할기본값;
*/
...
-- DPET_COPY 테이블에 LNAME 컬럼의 기본값을 미국으로 변경
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT '미국';
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT NULL; -- DEFAULT 삭제
...
ALTER TABLE DEPT_COPY DROP COLUMN LNAME; -- 에러 발생
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_ID_PK PRIMARY KEY(DEPT_ID);
...
-- EMPLOYEE 테이블의 DEPT_CODE, JOB_CODE 컬럼에 
...
ALTER TABLE MEMBER ADD 
CONSTRAINT MEMBER_GRADE_ID_FK FOREIGN KEY(GRADE_ID) REFERENCES MEMBER_GRADE;
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_ID_PK;

-- DEPT_COPY 테이블의 DEPT_TITLE 컬럼에 UQ, NN 제약조건 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C008477;
...
*/
-- DEPT_COPY 테이블에서 SYS_C008474 제약조건명을 DEPT_ID_NN로 변경
ALTER TABLE DEPT_COPY RENAME CONSTRAINT 
SYS_C008474 TO DEPT_ID_NN;

-- DEPT_COPY 테이블에서 SYS_C008475 제약조건명을 LOCATION_ID_NN로 변경
ALTER TABLE DEPT_COPY RENAME CONSTRAINT 
SYS_C008475 TO LOCATION_ID_NN;
...
-- 2) 아래와 같이 옵션을 지정하면 부모 테이블을 삭제할 수 있다.
